...
После установки preview APK и выдачи разрешения Customer регистрирует Expo Push Token в Backend. В Admin → «Клиенты» доступна форма «Проверочный Push»: она отправляет сообщение только на один номер с активным устройством. Backend вызывает Expo Push Service, а браузер Admin не получает device token. Ответ `accepted` означает, что Expo принял ticket; это не является гарантией отображения на телефоне, поэтому provider errors сохраняются как delivery status. Для реального Android Push нужен preview/development APK: remote notifications недоступны в Expo Go на Android с SDK 53 и выше.
